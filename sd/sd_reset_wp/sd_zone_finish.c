...
int do_zone_finish_ioctl(const char * pathname, uint64_t lba, int do_ata)
{
	int rc = -4;
		...
		rc = ioctl(fd, SCSI_IOCTL_FINISH_ZONE, iolba);
		if (rc != -1) {
			fprintf(stderr, "%s finish zone %" PRIx64 "%s okay\n",
...
int main(int argc, char *argv[])
{
	...
	if (0 == rc) {
		if (0 == flags) {
			rc = do_zone_finish_ioctl(fname, lba, do_ata);
		} else {
