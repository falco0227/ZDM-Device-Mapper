...
int do_zone_close_ioctl(const char * pathname, uint64_t lba, int do_ata)
{
	int rc = -4;
		...
		rc = ioctl(fd, SCSI_IOCTL_CLOSE_ZONE, iolba);
		if (rc != -1) {
			fprintf(stderr, "%s close zone %" PRIx64 "%s okay\n",
...
int main(int argc, char *argv[])
{
	...
	if (0 == rc) {
		if (0 == flags) {
			rc = do_zone_close_ioctl(fname, lba, do_ata);
		} else {
